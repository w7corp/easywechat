...
use EasyWeChat\WeWork\User\Client;
...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpGet('user/get', ['userid' => 'overtrue'])->andReturn('mock-result')->once();
    ...
    public function testDelete()
    {
        ...
        $client->expects()->httpGet('user/delete', ['userid' => 'overtrue'])->andReturn('mock-result')->once();
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPost('user/batchdelete', ['useridlist' => ['overtrue', 'foo']])->andReturn('mock-result')->once();
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('user/convert_to_openid', ['userid' => 'overtrue', 'agentid' => null])
                        ->andReturn('mock-result')->once();
        $this->assertSame('mock-result', $client->userIdToOpenid('overtrue'));

        $client->expects()->httpPostJson('user/convert_to_openid', ['userid' => 'overtrue', 'agentid' => 39202])
            ->andReturn('mock-result')->once();
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('user/convert_to_userid', ['openid' => 'mock-openid'])
            ->andReturn('mock-result')->once();
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpGet('user/authsucc', ['userid' => 'overtrue'])->andReturn('mock-result')->once();
